...
ARGS_FILE_ALT="$(realpath "$(dirname -- "${BASH_SOURCE[0]}")")/array_01_${PROJECT}_ALT.txt"
...
# ========================================================================= #
# Generate Experiment 2                                                     #
# ========================================================================= #

# SWEEP UNSUPERVISED DO-ADA-TVAE WITH BEST SETTING FROM ABOVE, BUT WITH
# SMALLER Z_SIZE TO MATCH ORIG EXPERIMENTS AND IMPROVE DISENTANGLEMENT SCORES?
#   1 * (1*4*1*5) = 20
ARGS_FILE="$ARGS_FILE_ALT" gen_sbatch_args_file \
    +DUMMY.repeat=1 \
    +EXTRA.tags='sweep_dotvae_hard_params_9_long' \
    hydra.job.name="dotvae_hparams_9_long" \
    \
    run_length=long \
    metrics=all \
    \
    ...
    \
    framework.cfg.triplet_loss=triplet \
    framework.cfg.overlap_num=1024 \
    sampling=gt_dist__manhat_scaled \
    framework.cfg.ada_thresh_mode=dist \
    \
    framework=X--dotvae \
    ...
    \
    framework.cfg.overlap_loss=NULL \
    framework.cfg.overlap_mine_ratio=0.1 \
    framework.cfg.overlap_mine_triplet_mode=none,hard_neg,semi_hard_neg,hard_pos,easy_pos \
    \
    framework.cfg.overlap_augment_mode=augment \
    framework.cfg.overlap_augment=NULL
...
# -- part of the above sweep!
# 1 * (1*1*1*5) = 5
ARGS_FILE="$ARGS_FILE_ALT" APPEND_ARGS=1 ARGS_START_NUM=21 gen_sbatch_args_file \
    +DUMMY.repeat=1 \
    +EXTRA.tags='sweep_dotvae_hard_params_9_long_xy' \
    hydra.job.name="dotvae_hparams_9_long_xy" \
    \
    run_length=long \
    metrics=all \
    \
...
# RUN THE EXPERIMENT:
# clog_cudaless_nodes "$PARTITION" 86400 "C-disent" # 24 hours
ARGS_FILE="$ARGS_FILE_ALT" submit_sbatch_args_file
