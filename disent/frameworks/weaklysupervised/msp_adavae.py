...
from disent.frameworks.weaklysupervised.adavae import AdaVae
...
class MspAdaVae(AdaVae):
    ...
    def compute_loss(self, batch, batch_idx):
        ...
        loss = ave_recon_loss + ave_kl_reg_loss + ave_msp_loss
        # -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~- #
    ...
    # def encode(self, x):
    #     z = super(MspAdaVae, self).encode(x)
    #     return self._msp.latent_to_labels(z)
    #
    # def decode(self, x):
    #     z = super(MspAdaVae, self).encode(x)
    #     return self._msp.latent_to_labels(z)
